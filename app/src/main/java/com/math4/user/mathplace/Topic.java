...
//    private RewardedAd rewardedAd;
    ...
    public void data(String link,int position){
        ...
        if(true) {
            if (link.equals("Правильно")) {
                tabLayout.getTabAt(position).setIcon(R.drawable.question_green_2);
                task[position][4] = "true";
            } else {
                tabLayout.getTabAt(position).setIcon(R.drawable.question_red_2);
                task[position][4]="false";
            }
        }else{
            viewPager.setCurrentItem(position+1,true);
        }
    ...
    public static class MyAdapter extends FragmentPagerAdapter {
        ...
        public Fragment getItem(int position) {
                ...
                if (String.valueOf(task_1.get(1)).equals("theory")) {
                    theory fragmentSend2 = new theory();
                    ...
                    bundle.putInt("check", ((Long) ((ArrayList<Long>) Distrib.allInf.get(thisTheme)).get(position)).intValue());
                    bundle.putInt("solution", ((Long) ((ArrayList<Long>) Distrib.allInf.get(thisTheme + "Solution")).get(position)).intValue());
                    bundle.putInt("star", Integer.parseInt(task_1.get(2).toString()));
                    bundle.putInt("position", position);
                    bundle.putInt("position_for_title", taskPosition.get(position));
                    if (position == ((ArrayList<Long>) Distrib.allInf.get(thisTheme)).size()-1) {
...
//        imageButtonBookmark=findViewById(R.id.imageButtonBookmark);
    ...
    public void setAdap(){
        ...
        ArrayList taskRegistr = (ArrayList) Distrib.allInf.get(thisTheme);
        for (int i = 0; i < taskProgress.size(); i++) {
            if(((ArrayList) tema.get("task" + String.valueOf(i))).get(1).toString().equals("null")){
